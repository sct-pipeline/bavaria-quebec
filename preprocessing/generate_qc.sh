...
# Define variables
file_gt="${file}_acq-ax_lesions-manual_T2w"
file_seg="${file}_acq-ax_seg-manual_T2w"
...
# Run the QC
sct_qc -i ${PATH_DATA_PROCESSED}/${SUBJECT}/anat/${file_img}.nii.gz -s ${file_seg}.nii.gz -d ${file_gt}.nii.gz -p sct_deepseg_lesion -plane axial -qc ${PATH_QC} -qc-subject ${SUBJECT}
