...
for subject in tqdm(subjects, desc='Iterating over Subjects'):
    ...
    for ses_idx in range(1, num_sessions_per_subject+1):
        ...
        session = os.listdir(temp_subject_path)[0]
        ...
        img_path = os.path.join(subject_images_path, '%s_%s_acq-ax_T2w.nii.gz' % (subject, session))
        img_crop_fpath = os.path.join(subject_images_path, '%s_%s_acq-ax_T2w_crop.nii.gz' % (subject, session))
        ...
        gt_fpath = os.path.join(subject_labels_path, '%s_%s_acq-ax_lesion-manual.nii.gz' % (subject, session))
        gt_crop_fpath = os.path.join(subject_labels_path, '%s_%s_acq-ax_lesion-manual_crop.nii.gz' % (subject, session))
