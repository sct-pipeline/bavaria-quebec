...
# sct_deep_seg for cropped, axial images with thick slices using the t2 setting
# sct_deepseg_lesion -i ${file}_T2w_crop_res.nii.gz -c t2 -centerline file -file_centerline ${file_seg}_res.nii.gz -o lesion_t2ax
sct_deepseg_lesion -i ${file}_T2w_crop_res.nii.gz -c t2 -o lesion_t2ax
...
rsync -avzh $PATH_DATA_PROCESSED/derivatives/labels/${SUBJECT}/anat/${file_gt}_crop_res.nii.gz $PATH_DATA_PROCESSED_CLEAN/derivatives/sct-5.8-isotropic/labels/${SUBJECT}/anat/${file_gt}.nii.gz
rsync -avzh $PATH_DATA_PROCESSED/derivatives/labels/${SUBJECT}/anat/${file_gt}.json $PATH_DATA_PROCESSED_CLEAN/derivatives/sct-5.8-isotropic/labels/${SUBJECT}/anat/${file_gt}.json
rsync -avzh $PATH_DATA_PROCESSED/${SUBJECT}/anat/lesion_t2ax/${file}_T2w_crop_res_lesionseg.nii.gz $PATH_DATA_PROCESSED_CLEAN/derivatives/sct-5.8-isotropic/labels/${SUBJECT}/anat/${file}_lesion-processed_T2w.nii.gz
